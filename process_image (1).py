...
def find_from_to_xy(img, line):
    ...
    # y = ax + b
    x1, y1 = int(x-vx*int(ht/3)), int(y-vy*int(ht/3))
    x2, y2 = int(x+vx*int(ht/3)), int(y+vy*int(ht/3))

    # kiem tra bien
    x1 = x1 if x1 <= wd else x1 if x1 >= 0 else 0
    x2 = x2 if x2 <= wd else x2 if x2 >= 0 else 0
    y1 = y1 if y1 <= ht else y1 if y1 >= int(ht/3) else int(ht/3)
    y2 = y2 if y2 <= ht else y2 if y2 >= int(ht/3) else int(ht/3)
    return x1, y1, x2, y2
